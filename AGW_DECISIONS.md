...
## ADR-026: Member Research Stays Static — No Backend, Ever

**Date:** 2026-07-11
**Status:** Accepted

**Decision:**
"Forschung der Mitglieder" is a static data file (`agw_member_pubs.js`) plus structured intake plus maintainer-side automation (`tools/pubs_import.py`, `tools/orcid_seed.py`), on an **annual cycle**: one email before each Jahrestagung → one script run → one commit. No backend, no database, no auth.

**Rationale:**
Three alternatives were rejected. *Hand-curation as the steady state*: 48 members × ~3 items/yr ≈ 150 edits/yr, all falling on one person, and the page's entire value is its freshness. *GitHub-PR intake as the primary channel*: realistic uptake among this membership is roughly three people — document it, don't build for it. *Any live backend*: auth, moderation, spam, a data-processor agreement, recurring cost — and it breaks the no-server property the whole site rests on. Decisively: **this is a VfS committee site, not a personal project.** A backend the current maintainer personally hosts becomes an institutional liability the day he hands it over.

**Consequences:**
- Every intake path must terminate in a file diff, not a write.
- The annual cycle is the *only* guaranteed refresh; the page must read as a *selection* (ADR-029), not a live CV.
- Tooling is therefore load-bearing and belongs in the repo under `tools/`.

---

## ADR-027: No LLM-Generated Biographies of Living, Named Colleagues

**Date:** 2026-07-11
**Status:** Accepted

**Decision:**
No biographical prose about a member is ever generated by a language model. The site links to the member's own institutional page and ORCID, and shows the research foci already recorded in `MEMBERS`. `tools/cv_extract.py` performs extraction only; the prohibition is written into its docstring so a future session does not cheerfully reinstate it.

**Rationale:**
A fluent, confident, subtly wrong sentence about a colleague's career, published under the committee's name, is both a DSGVO Art. 5(1)(d) accuracy failure and a collegial one. It is exactly the error that survives review *because* it reads correct.

**Consequences:**
- Member cards carry facts, not narrative.
- Where a biography is genuinely wanted, it is requested from the member.

---

## ADR-028: `MEMBER_PUBS` Keyed on the Stable `mid` Slug, Never the Display Name

**Date:** 2026-07-11
**Status:** Accepted

**Decision:**
Every `MEMBER_PUBS` entry (and every `mids[]` entry in `VOLUME_CHAPTERS`) references a member by the stable `id` slug on `MEMBERS`. A name-based fallback is retained in the app so a legacy hand-added entry still resolves, but new data is never keyed on a name.

**Rationale:**
Names drift — `Heinz D. Kurz` / `Heinz Kurz`, `Allgöwer` / `Allgoewer`, `Hans Frambach` / `Hans A. Frambach`. Slugs do not. Slugs are opaque and **must not churn**: `frambach-hans` stays correct even if the display name is corrected to `Hans A. Frambach`.

**Consequences:**
- Renaming a member is a one-line display change, never a data migration.
- Author→member matching (surname + first initial, umlaut/ß folded) lives in `pubs_import.py::bib_works` and is reused by every importer.

---

## ADR-029: No Vitae, No CV Hosting — An Explicitly Selective Bibliography

**Date:** 2026-07-11
**Status:** Accepted

**Decision:**
The minimum-risk personal data set: name, institution, research foci, *selected* publications, ORCID/homepage links. No CVs, no vitae, no employment histories. The page is framed on its face as selective — "ausgewählte Publikationen" / "selected publications".

**Rationale:**
Honest and self-limiting at once. It caps the maintenance obligation (nobody can complain a *selection* is incomplete) and keeps the processed data to the minimum the purpose requires.

**Consequences:**
- Intake asks for an **ORCID iD, not a CV**.
- Completeness is explicitly not promised, which is what makes the annual cycle (ADR-026) sufficient.

---

## ADR-030: The Tagungsband Chapter Corpus — D&H eLibrary as Source of Truth

**Date:** 2026-07-12
**Status:** Accepted

**Decision:**
All 288 chapters of the 43 Tagungsbände are harvested from the **Duncker & Humblot eLibrary HTML**, cross-checked against the PDF bookmark outlines, and emitted to `agw_volume_chapters.js` as `VOLUME_CHAPTERS` (+ `VOLUME_META`). The corpus is kept **separate from `MEMBER_PUBS`** — different provenance, different legal basis, different rendering. `mids` is a **list** and may be empty: most contributors are guests, not members.

**Rationale:**
Crossref was tested first and rejected: it holds chapter DOIs for only two of the 43 volumes, print ISBNs return nothing, and `query.container-title` is a relevance *ranking*, not a filter — a series-wide sweep returned D&H's entire corpus across unrelated series (476 items), a false ceiling that briefly looked like a result. EconStor's web UI is behind an anti-bot wall and will not be engineered around. The publisher's own eLibrary HTML is complete, and its per-book **citation block** (APA/MLA/Harvard) is the only reliable source for imprint year and editors: `PUBLICATIONS` has `year:null` for **31 of 43** volumes, and the `citation_publication_date` meta tag is the *e-edition* date (Band X reports 2021; Band XX reports 2015).

**Consequences:**
- `tools/themes.csv` is the **curation overlay** — a per-chapter theme assignment keyed `volN|pages`, read by `dh_toc.py` at build time. It is the single most valuable human artefact in this pipeline: **without it in the repo, a rebuild regenerates every theme as `GUESSED` and silently destroys the curation.** It must be committed and must never be gitignored.
- PDF filenames carry **stale band numbers** (IX/X and XXXV/XXXVII are swapped). PDFs are matched to volumes by *content fingerprint* (Jaccard over chapter titles), never by filename — and their year prefixes are therefore also unsafe.
- The corpus makes AGW contributions the **spine** of a member's record and personal publications the supplement. 47 of 48 members appear in it.
- The consent problem largely dissolves for the core content: a member's chapter in the committee's own proceedings *is* the committee's own publication record.

---

## ADR-031: The Member Bibliography Is Committee-Curated (Legitimate Interest), Not Consent-by-Submission

**Date:** 2026-07-12
**Status:** Accepted — **supersedes the standing rule in `pubs_import.py`**

**Decision:**
`tools/pubs_import.py` previously carried the rule: *"Only run this for a member who has SENT you their ORCID iD. Consent comes from the submission, not from the data being findable."* That rule is **withdrawn**. The member bibliography is a **committee-curated scholarly record**, assembled from data the members themselves made public (ORCID, Crossref, publisher metadata), on a **legitimate-interest** basis. `orcid_seed.py --find` may therefore search ORCID by name.

**Rationale:**
The rule no longer described the site. The 288 Tagungsband chapters are the committee's own publication record — nobody submitted them. The 2026 supplements were curated from the maintainer's own `.bib` — nobody submitted those either. Enforcing a consent-by-submission rule on the ORCID path alone would have been a distinction without a difference, applied to the one channel that happened to have a docstring.

**Consequences — these are obligations, not notes:**
1. **A Datenschutzerklärung must exist.** Legitimate interest without a privacy notice is not a legal basis, it is a preference. Impressum + Datenschutzerklärung are now a **prerequisite** for running the ORCID harvest at scale, not a follow-up task.
2. **An opt-out must be offered and honoured without argument.**
3. **Accuracy is on the maintainer** (DSGVO Art. 5(1)(d)). Every line is read before it ships. Homonyms (`Helmut Wagner`) and stale working papers are data-protection failures, not typos.
- Both tools carry this policy in their docstrings, so the next session inherits the decision rather than re-litigating it.

---

## ADR-032: Chapter-Level Citation Export — Chicago, Harvard, BibTeX (amends ADR-008)

**Date:** 2026-07-12
**Status:** Accepted

**Decision:**
`agw_cite.js` provides per-chapter citation export in **Chicago, Harvard, and BibTeX**, plus bulk `.bib` download per volume (`data-cite-all="vol:X"`) and per member (`data-cite-all="mid:X"`). Entirely client-side: a Blob + object URL, with an `execCommand` clipboard fallback for contexts where the async Clipboard API is unavailable.

**Rationale:**
ADR-008 chose four formats (BibTeX, EndNote, RIS, Chicago) for *volume-level* citation. At chapter level the audience is a working historian of economic thought, for whom Harvard (author–date) matters more than EndNote/RIS export files. This **amends** ADR-008 for the chapter case; the volume-level exporter is untouched.

**Consequences:**
- Citation quality depends entirely on `VOLUME_META` (ADR-030). Without it, every citation reads *n.d.* — which is exactly what happened when the `window.AGW_DATA.VOLUME_META` export was accidentally omitted.
- The publisher supplies editors APA-inverted (`Neumark, F.`). Harvard wants exactly that; **Chicago wants natural order** (`edited by F. Neumark`). The un-inversion is deliberate, not incidental.
- German volume titles, series, publisher and place are **not** translated in EN mode — same rule as speaker names and venues (ADR-014).

---

## ADR-033: Aggregate Views Count the Corpus; Member Views Count the Expansion

**Date:** 2026-07-12
**Status:** Accepted

**Decision:**
Any statistic, filter, or aggregate view over the chapter corpus is computed from **`VOLUME_CHAPTERS` itself** — one row per chapter. The per-member expansion (`MEMBER_CHAPTERS`, one row per member author) is used **only** where the unit of display genuinely is the member.

**Rationale:**
Two bugs in one day, both from the same mistake, both **silent**:
- The **Referenten tab** ranked contributors from `ARCHIVE[].papers[]` — 150 talks from only 18 of 46 conferences. It omitted 29 people with ≥2 published chapters, including **Dieter Schneider (12 chapters), Karl-Heinz Schmidt (10), Heinz Rieter (8)**. It rendered a confident, plausible, wrong ranking with the committee's most prolific contributor missing.
- The **theme view** on `publications-members.html` counted `MEMBER_CHAPTERS`, so the one chapter co-authored by two members was listed twice, and all **145 guest-authored chapters vanished** — every theme pill undercounted (`general` 33 of 59, `spatial` 11 of 22).

Neither threw an error. Both looked right. The themes describe the **corpus**, not the membership: a chapter by a guest author is still a chapter on that theme.

**Consequences:**
- `agw_member_pubs_app.js` exposes `rowsFor(view)`; `allRows()` is view-aware.
- Contributor statistics fold names before counting (surname + first initial, umlaut/ß normalised, nobiliary particles kept with the surname) — the corpus spells the same person several ways (`Erich Streißler` / `Erich W. Streissler`, `Jürgen Backhaus` / `Jürgen G. Backhaus`).
- **A wrong number is more dangerous than a crash.** A guard that returns empty (ADR-016) protects the page but hides the fault; prefer an assertion against the corpus in any new aggregate view.
